...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "8240385259:AAH5-3ClYv9eGryOBvxAVApT39SUjtX4ZVs")
    API_ID = int(os.environ.get("API_ID",  "33041699"))
    API_HASH = os.environ.get("API_HASH", "04cc61b089c1f7a79b6d508759c16678")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '8024758566').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1003624610812').split(',')
                                         
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1003254915631")
